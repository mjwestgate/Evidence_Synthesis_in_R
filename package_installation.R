...
packages <- read.csv("./Evidence_Synthesis_in_R/data-raw/packages.csv",
...
revtools_data <- get_pkg_docs("metafor")
class(revtools_data) # list
revtools_df <- list_to_df(revtools_data)
class(revtools_df) # data.frame

# get a df for all packages
function_df <- create_function_df(
  packages$package_name[which(packages$installed)]
)
# add a column that groups package and function names
function_df$function_detailed <- paste(
  function_df$package_name,
  function_df$function_name,
  sep = "::"
)

# map with topic models
library(revtools)
screen_topics(function_df)